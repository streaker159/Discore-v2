...
} = require("../../../modules/gameData/unitImportService");
...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((s) =>
      s
        .setName("sync-game")
        .setDescription("Sync all unit data for a game from wiki API.")
        .addStringOption((o) =>
          o
            .setName("game")
            .setDescription("Game to sync")
            .setRequired(true)
    ...
    .addSubcommand((s) =>
      s
        .setName("sync-page")
        .setDescription("Sync a single wiki page.")
        .addStringOption((o) =>
          o
            .setName("game")
            .setDescription("Game")
        ...
        .addStringOption((o) =>
          o
            .setName("page")
            .setDescription("Wiki page title (e.g. Mobile Radar)")
            .setRequired(true),
        ),
    )
    .addSubcommand((s) =>
    ...
    .addSubcommand((s) =>
      s
        .setName("preview")
        .setDescription("Preview pending import drafts.")
        .addStringOption((o) =>
          o
            .setName("game")
    ...
    .addSubcommand((s) =>
      s
        .setName("approve")
        .setDescription("Approve a draft into the verified database.")
        .addStringOption((o) =>
          o
            .setName("draft_id")
            .setDescription("Draft ID to approve")
            .setRequired(true),
        ),
    )
    .addSubcommand((s) =>
      s
        .setName("reject")
        .setDescription("Reject a draft.")
        .addStringOption((o) =>
          o
            .setName("draft_id")
            .setDescription("Draft ID to reject")
            .setRequired(true),
        ),
    )
    .addSubcommand((s) =>
  ...
  async execute(interaction) {
    ...
    if (sub === "init") {
      ...
      return interaction.editReply({
        content: "✅ Game data sources initialized.",
      });
    }
    ...
    if (sub === "sync-game") {
      ...
      const embed = new EmbedBuilder()
        .setTitle("📚 Unit Data Sync Complete")
        .setColor(0x1a7a9e)
        .addFields(
          ...
          {
            name: "Tables Found",
            value: String(result.tablesFound),
            inline: true,
          },
          {
            name: "Draft Units Created",
            value: String(result.draftCount),
            inline: true,
          },
          {
            name: "Low Confidence",
            value: String(result.lowConfidence),
            inline: true,
          },
        )
        .setFooter({ text: "Use /unitdata preview to review drafts" })
        .setTimestamp();
    ...
    if (sub === "sync-page") {
      ...
      if (!result.ok)
        return interaction.editReply({
          content: `❌ Page sync failed: ${result.error}`,
        });
      return interaction.editReply({
        content: `✅ Page **${result.page}** synced. ${result.draftCount} drafts created (${result.lowConfidence} low confidence).`,
      });
    }
    ...
    if (sub === "status") {
      ...
      if (statuses.length === 0)
        return interaction.editReply({
          content:
            "No game data sources configured. Run `/unitdata init` first.",
        });

      const embed = new EmbedBuilder()
        .setTitle("📊 Unit Data Status")
        .setColor(0x1a7a9e);
    ...
    if (sub === "preview") {
      ...
      const drafts = gameKey
        ? await repo.getDrafts(gameKey)
        : await repo.getAllDrafts();
      if (drafts.length === 0)
        return interaction.editReply({
          content: "No pending drafts to preview.",
        });

      const embed = new EmbedBuilder()
        .setTitle(`📋 Pending Drafts (${drafts.length})`)
        .setColor(0x1a7a9e);
      ...
      if (drafts.length > 10)
        embed.setFooter({ text: `Showing 10 of ${drafts.length} drafts` });
      return interaction.editReply({ embeds: [embed] });
    }
    ...
    if (sub === "approve") {
      ...
      if (draftId.toLowerCase() === "all") {
        ...
        if (drafts.length === 0)
          return interaction.editReply({
            content: "No pending drafts to approve.",
          });
        let ok = 0,
          fail = 0;
        ...
        return interaction.editReply({
          content:
            `✅ Approved ${ok} drafts across all games.` +
            (fail ? ` ${fail} failed.` : ""),
        });
      }
      await interaction.deferReply({ flags: [MessageFlags.Ephemeral] });
      ...
      if (!result.ok)
        return interaction.editReply({
          content: `❌ Approval failed: ${result.error}`,
        });
    ...
    if (sub === "reject") {
      ...
      return interaction.editReply({
        content: `🚫 Draft rejected: ${draft.parsedJson?.name || "Unknown"}`,
      });
    }
  },
};
