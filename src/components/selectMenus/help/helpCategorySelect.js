...
async function buildOverviewEmbed(interaction) {
  return createDiscoreEmbed(interaction, {
    ...
    description: [
      ...
      "`/server` · `/premium` · `/safecrack` · `/ping` · `/help` · `/xp` · `/autopost`",
      "",
      "### 🎯 Supported Games",
...
async function buildPremiumEmbed(interaction) {
  return createDiscoreEmbed(interaction, {
    ...
    description: [
      ...
      "• Any server can request extra AI credits directly from the Discore owner — no Premium required",
      ...
      "• Contact the Discore owner for Premium costs, rates, and deals",
      ...
      "",
      "### 💰 How to Upgrade",
      "Discore no longer uses the Discord Shop for premium. To get Discore Premium, contact the Discore owner directly or ask in the official Discore server for current costs and rates.",
      "• Payment is handled directly outside the Discord Shop",
      "• Deals can be arranged for certain servers or people",
      "• If you receive a code, run `/premium` and press **Redeem Code**",
    ].join("\n"),
  });
}
...
async function buildAutoPostsEmbed(interaction) {
  return createDiscoreEmbed(interaction, {
    ...
    description: [
      ...
      "",
      "### 📋 Trigger Types",
      "**⏰ Scheduled** — Send at specific times (once, daily, weekly, monthly, or every X hours). Perfect for daily reminders, weekly announcements, monthly recaps.",
      "",
      "**👋 Member Join** — Welcome new members automatically when they join. Includes a Yes/No toggle to enable/disable sending. Supports member-specific placeholders like `{user_mention}` and `{display_name}`.",
      "",
      "**💬 Mention Trigger** — Reply when a role/user is mentioned. Configurable cooldown to prevent spam.",
      "",
      "**🔑 Keyword/Phrase** — Reply when text appears in chat (CONTAINS or EXACT match). Configurable cooldown.",
      "",
      "### 📝 Welcome Template — Full Example",
      "To create a member-join welcome message, run `/autopost`, click **➕ Create**, select **👋 Member Join** trigger, choose **BOTH** message mode, then fill in:",
    ].join("\n"),
    fields: [
      {
      ...
      },
      {
        ...
        value: [
          ...
          "|---|---|",
          "| `{user_mention}` | Discord ping for the joining user |",
      ...
      {
        name: "👋 Member Join Settings",
        value: [
          "When creating a Member Join auto post, you'll see the **Member Join Settings** panel:",
          "",
          ...
          "",
          "Toggle this anytime by editing the post.",
        ].join("\n"),
        inline: false,
      },
      {
        ...
        value: [
          ...
          "• `allowed_mentions` locked down: only the joining user can be pinged",
      ...
      },
      {
        ...
        value: [
          ...
          "• Put `{user_mention}` in the **message content** for a real ping notification",
          "• Use `<#channel_id>` in descriptions for clickable channel links",
          "• Click **📝 View Placeholders** in the dashboard for a quick reference",
      ...
      },
      {
        ...
        value: [
          ...
          "• Message content: 1,900 chars max",
          "• Embed title: 256 chars max",
          "• Embed description: 4,000 chars max",
          "• Footer: 2,048 chars max",
        ].join("\n"),
        inline: false,
      },
    ],
