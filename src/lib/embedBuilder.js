...
async function createDiscoreEmbed(interactionOrGuildId, options = {}) {
  ...
  const client = typeof interactionOrGuildId === 'string'
    ? options.client
    : interactionOrGuildId?.client;

  const settings = options.guildSettings || await getGuildSettings(guildId);
  const allianceName = settings?.allianceName || options.allianceName || 'Discore';
  const allianceLogo = settings?.allianceLogo || options.allianceLogo || undefined;
