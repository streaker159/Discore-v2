...
function participantLines(signup, status) {
  const users = signup.participants.filter((p) => p.status === status).map((p, idx) => `${idx + 1}. <@${p.userId}>`);
...
async function buildBattleSignupEmbed(interactionOrGuildId, signup) {
  ...
  return createDiscoreEmbed(interactionOrGuildId, {
    ...
    fields: [
      ...
      { name: 'Accepted', value: participantLines(signup, 'ACCEPTED'), inline: true },
      { name: 'Reserves', value: participantLines(signup, 'RESERVE'), inline: true },
      { name: 'Declined', value: participantLines(signup, 'DECLINED'), inline: true },
    ],
  });
}
...
function battleSignupButtons(signupId) {
  return [
    new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId(`battle:join:${signupId}`).setLabel('Join').setEmoji('✅').setStyle(ButtonStyle.Success),
      new ButtonBuilder().setCustomId(`battle:reserve:${signupId}`).setLabel('Reserve').setEmoji('🪑').setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId(`battle:decline:${signupId}`).setLabel('Decline').setEmoji('❌').setStyle(ButtonStyle.Danger),
      new ButtonBuilder().setCustomId(`battle:remind:${signupId}`).setLabel('Remind me').setEmoji('🔔').setStyle(ButtonStyle.Primary),
    ),
  ];
}
