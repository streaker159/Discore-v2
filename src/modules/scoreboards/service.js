...
async function createScoreboard({ guildId, name, metric, type, channelId, createdBy }) {
  ...
  if (activeCount >= plan.limits.liveScoreboards) {
    throw new Error(`This server has reached its live scoreboard limit for ${plan.tier}: ${plan.limits.liveScoreboards}.`);
  }
...
async function addResult({ guildId, scoreboardName, targetId, targetType = 'USER', action, delta = 1, adminId, reason }) {
  ...
  const entry = await prisma.scoreboardEntry.upsert({
    ...
    update: action === 'WIN'
      ? { wins: { increment: delta }, winStreak: { increment: delta }, lossStreak: 0 }
      : action === 'LOSS'
        ? { losses: { increment: delta }, lossStreak: { increment: delta }, winStreak: 0 }
...
async function archiveScoreboard({ guildId, name }) {
  ...
  return prisma.scoreboard.update({ where: { id: board.id }, data: { isArchived: true } });
}
...
async function buildScoreboardEmbed(interaction, board) {
  ...
  return createDiscoreEmbed(interaction, {
    ...
    fields: [
      ...
      { name: 'Archived', value: board.isArchived ? 'Yes' : 'No', inline: true },
    ],
  });
}
