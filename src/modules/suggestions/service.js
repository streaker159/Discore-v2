...
async function getSuggestion(id) {
  return prisma.suggestion.findUnique({ where: { id }, include: { votes: true } });
}
...
function suggestionButtons(suggestionId) {
  return [new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId(`suggestion:up:${suggestionId}`).setLabel('Upvote').setEmoji('👍').setStyle(ButtonStyle.Success),
    new ButtonBuilder().setCustomId(`suggestion:down:${suggestionId}`).setLabel('Downvote').setEmoji('👎').setStyle(ButtonStyle.Danger),
  )];
}

module.exports = { createSuggestion, vote, getSuggestion, buildSuggestionEmbed, suggestionButtons };
