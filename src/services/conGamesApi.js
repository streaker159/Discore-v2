...
/**
 * Build the endpoint URL for player lookups.
 */
function buildPlayerEndpoint() {
  const url = new URL("https://www.conflictnations.com/index.php");

  url.search = new URLSearchParams({
    action: "getUserDetailsFirefly",
    eID: "api",
    key: "ingameCon",
    hash: process.env.CON_GAMES_HASH,
    outputFormat: "json",
...
/**
 * Build the decoded payload for a player lookup.
 * @param {{ userId?: string, username?: string }} params
 */
function buildPlayerDecodedPayload({ userId, username }) {
  const values = [
    ["regTstamp", "1"],
    ["scenarioStats", "1"],
    ["gameStats", "1"],
  ...
  if (userId) {
    values.unshift(["userID", String(userId)]);
  } else if (username) {
    values.unshift(["username", username]);
  }
...
/**
 * Fetch a Conflict of Nations player profile.
 * @param {{ userId?: string, username?: string }} lookup
 * @returns {Promise<object>} — the player object from responseData.result
 */
async function fetchPlayer({ userId, username } = {}) {
  if (!userId && !username) {
    throw new Error("Player lookup requires userId or username.");
  }

  // ── Validate configuration ───────────────────────────────────────
  const missing = getMissingEnvVars();
  if (missing.length > 0) {
    const msg = `Conflict of Nations player lookup is not configured. Missing ${missing.join(", ")}.`;
    throw new Error(msg);
  }

  // ── Build request ────────────────────────────────────────────────
  const endpoint = buildPlayerEndpoint();
  ...
  try {
    ...
    clearTimeout(timeout);
    if (err.name === "AbortError") {
      throw new Error("Player lookup timed out");
    }
    throw err;
  } finally {
    clearTimeout(timeout);
  ...
  if (response.status === 429) {
    const retryAfter =
      ...
      response.headers.get("retry-after");
    const waitSec = retryAfter ? parseInt(retryAfter, 10) : 60;
    throw new Error(`HTTP 429: rate-limited (retry after ${waitSec}s)`);
  }
  ...
  try {
    ...
    throw new Error("Failed to parse player API response as JSON");
  }

  if (bodyData == null || typeof bodyData !== "object") {
    throw new Error("Player API returned a non-object response");
  }

  logger.debug("Player lookup: API response", {
    resultCode: bodyData.resultCode,
    resultMessage: bodyData.resultMessage,
  });
  ...
  if (Number(bodyData.resultCode) !== 0) {
    throw new Error(
      `Player API returned resultCode ${bodyData.resultCode}: ${bodyData.resultMessage}` +
        (bodyData.result
          ? ` Action result: ${JSON.stringify(bodyData.result)}`
          : ""),
    );
  ...
  if (!bodyData.result || typeof bodyData.result !== "object") {
    throw new Error("Player API response missing result object");
  }

  return bodyData.result;
}

module.exports = { fetchGames, fetchPlayer, getMissingEnvVars };
